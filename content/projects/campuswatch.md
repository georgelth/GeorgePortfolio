...
# the idea
Campus life involves a surprising number of disconnected systems. Students check one place for their classes, another for weather, text friends to organize study sessions, and often rely on word of mouth for things happening around campus. CampusWatch started from the idea that some of those experiences could live together in one place.
...
# Study Groups
One of the largest features I worked on was the study group system. Students could create a study session for one of their courses, give it a name and location, set a maximum number of participants, and begin hosting it. Once started, the app transitioned into a live host view where the organizer could see information such as the group's name, participant count, and how long the session had been active.
...
# Attendance Coach
The Attendance Coach explored how location could be used to give students a better picture of their attendance habits. Using geolocation, the application could determine whether a student was present where they were expected to be and use that information to provide attendance-related feedback.
...
# Safety Navigator
CampusWatch also incorporated weather and safety information using data from the National Weather Service. The application could retrieve forecasts and active alerts, including watches, warnings, and advisories, and present information based on the severity of the event.
...
# Connecting Students
Another part of CampusWatch focused on connecting students through the classes they were already taking. Using enrollment data, the application could identify a student's courses and classmates, giving other features like study groups a natural connection to the student's actual schedule.

This helped CampusWatch feel less like a collection of unrelated utilities and more like one connected application. Course information could become context for other features instead of forcing the student to repeatedly enter the same information.
