            ...
            {Object.entries(navItems).map(([path, { name }]) => {
              return (
                <Link
                  key={path}
                  href={path}
                  className="transition-all hover:text-neutral-200 flex align-middle relative py-1 px-2 m-1 underline decoration-transparent hover:decoration-gray-400 duration-200 underline-offset-2"
                >
                  {name}
                </Link>
              )
